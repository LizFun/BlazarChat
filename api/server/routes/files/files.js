...
router.get('/download/:fileId', async (req, res) => {
  try {
    ...
    const fileResponse = await axios.get(`https://new_api.blazarpowered.com/v1/files/${fileId}`, {
      headers: options.headers,
    });
    const { filename } = fileResponse.data;

    const response = await axios.get(`https://new_api.blazarpowered.com/v1/files/${fileId}/content`, options);
